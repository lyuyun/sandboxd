...
systemd                node-proxy worker               node-ctl serve              sandbox-ctl
   ...
   │
   │  node-proxy        vswitch (GENEVE/tap)       cloud-hypervisor   guest user:PORT
   │        │                   │                       │                  │
   │        │ KindTCP:          │                       │                  │
...
← io.Copy 双向 splice 贯通：客户端 ↔ node-proxy ↔ vswitch tap → CH virtio-net → guest 用户 app
```
...
transit NIC (eth1)  [TC hook: prog_tc_ingress]
  ...
  ▼
sandbox-vswitch  [sw0_vswitch netns，GENEVE overlay]
  │  TC eBPF → tap fd → CH virtio-net → guest
  ▼
guest 用户 app
...
> **与 proxy 的关系**：用户端口流量（非 49983/49999）经 node-proxy 首次建立后，`dst=floating_ip:port` 数据包沿 GENEVE/tap → CH virtio-net 通路直达 guest user app；sandbox-ctl 不参与此路径；proxy 完成 L7 鉴权后退出连接热路径（FE-7.2 flowtable 接管后完全绕过 proxy 用户态）。
...
|---------|-----------|----------------|------------|---------|
| 用户端口（非 49983/49999）| node-proxy → floatingip:PORT → vswitch GENEVE/tap → CH virtio-net → guest | ❌ 不经手 | ✅ tap 模式 | 网络包，走 TC eBPF + GENEVE |
