...
│  │  │  │                                                                                              │  │  │   │
│  │  │  │  user app      :PORT    floatingip:PORT ◄─── node-proxy TCP splice                          │  │  │   │
...
│  │  │  │                                                                                              │  │  │   │
│  │  │  │  出站流量      默认路由 → GENEVE → transit NIC (eth1) → 外部网关 / 跨宿主 overlay             │  │  │   │
│  │  │  └──────────────────────────────────────────────────────────────────────────────────────────── ─┘  │  │   │
│  │  └────────────────────────────────────────────────────────────────────────────────────────────────────┘  │   │
│  └──────────────────────────────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                                                 │
...
| floatingip:PORT | TCP | sandbox floatingip:PORT（vswitch GENEVE 路由）| proxy splice → guest user app | X-Access-Token (proxy 层) |
...
sandbox-vswitch  [sw0_vswitch netns，GENEVE overlay]
  │  按 FloatingIP 路由，GENEVE 封装发往 guest TAP
  ▼
guest 网卡 (TAP, cloud-hypervisor)
  ▼
用户 app  [guest 内，监听 :PORT]

← io.Copy 双向 splice 贯通：客户端 ↔ node-proxy ↔ vswitch ↔ guest 用户 app
```
...
transit NIC (eth1)  [TC hook: prog_tc_ingress]
  ...
  │  内核直接改写目标地址为 FloatingIP:PORT
  │  node-proxy 用户态 splice 循环退出（不再搬运字节）
  ▼
sandbox-vswitch  [sw0_vswitch netns，GENEVE overlay]
  ▼
guest 用户 app
...
gateway → transit NIC → TC ingress eBPF：
  ...
  → sw0-tN → cloud-hypervisor → guest
```

> **与 proxy 的关系**：用户端口流量（非 49983/49999）经 node-proxy 首次建立后，`dst=floating_ip:port` 数据包沿 GENEVE 通路直达 guest 内部服务；proxy 完成 L7 鉴权后退出连接热路径（FE-7.2 flowtable 接管后完全绕过 proxy 用户态）。
...
| 用户端口数据面通路 | GENEVE 隧道将 floating_ip:port 流量送达 guest 内部服务 |
...
| 端口类型 | 数据面路径 | vswitch 参与 | 底层传输 |
|---------|-----------|------------|---------|
| 用户端口（非 49983/49999）| node-proxy → floatingip:PORT → vswitch GENEVE → guest | ✅ tap 模式 | 网络包，走 TC eBPF + GENEVE |
| envd/ci 端口（49983/49999）| node-proxy → envd.sock → Forwarder → vsock.sock → CH virtio-vsock → vsock relay → envd | ❌ 不经过 vswitch | virtio-vsock virtqueue，共享内存 |

vsock 由 cloud-hypervisor 实现，走 virtio-vsock virtqueue（共享内存），与 vswitch 的 tap 设备、TC eBPF 程序、GENEVE 隧道完全无关。两者是并行的两套 host↔guest 通道，面向不同用途：vswitch tap 承载对外可路由的网络流量，vsock 承载 sandbox-ctl 与 guest 之间的内部 IPC（含 envd/ci 数据面 relay）。
